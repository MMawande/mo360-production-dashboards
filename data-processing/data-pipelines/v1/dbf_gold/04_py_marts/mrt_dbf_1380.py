...
tra_dbf_1380_main = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_1380_main"
)
tra_dbf_1380_korrelation = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_1380_korrelation"
)
tra_dbf_1380_karnrresult = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_1380_karnrresult"
)
tra_dbf_1380_additionalchecks = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_1380_additionalchecks"
)
tra_dbf_1380_mrs = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_1380_mrs"
)
...
# Test data source
tra_dbf_1380_korrelation.display()
...
t1 = (
    tra_dbf_1380_main.filter(col("ingest_date") >= date_sub(current_date(), 90))
...
mrt_dbf_1380 = (
    t1.join(
        tra_dbf_1380_karnrresult.alias("t2"),
        (
    ...
    .join(
        tra_dbf_1380_korrelation.alias("t3"),
    ...
    .join(
        tra_dbf_1380_additionalchecks.alias("t4"),
...
mrt_dbf_1380.display()

# COMMAND ----------

# Select final fields and cast types
mrt_dbf_1380_final = mrt_dbf_1380.select(
    "anlage_subtyp",
    "timestamp",
...
result_df = mrt_dbf_1380_final.unionByName(tra_dbf_1380_mrs)
...
result_df.write.saveAsTable(
    f"`{catalog}`.`{marts_schema}`.mrt_dbf_1380", mode="overwrite"
)
