...
tra_dbf_0670_main = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0670_main"
)
tra_dbf_0670_korrelation = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0670_korrelation"
)
tra_dbf_0670_karnrresult = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0670_karnrresult"
)
tra_dbf_0670_additionalchecks = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0670_additionalchecks"
)
tra_dbf_0670_mrs = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0670_mrs"
)
...
# Test data source
tra_dbf_0670_korrelation.display()
...
t1 = tra_dbf_0670_main.filter(col("ingest_date") >= date_sub(current_date(), 90)).alias("t1")

# reference t1.columns
mrt_dbf_0670 = (
    t1.join(
        tra_dbf_0670_karnrresult.alias("t2"),
        (
    ...
    .join(
        tra_dbf_0670_korrelation.alias("t3"),
    ...
    .join(
        tra_dbf_0670_additionalchecks.alias("t4"),
...
mrt_dbf_0670.display()

# COMMAND ----------

# Select final fields and cast types
mrt_dbf_0670_final = mrt_dbf_0670.select(
    "anlage_subtyp",
    "timestamp",
...
result_df = mrt_dbf_0670_final.unionByName(tra_dbf_0670_mrs)
...
result_df.write.saveAsTable(
    f"`{catalog}`.`{marts_schema}`.mrt_dbf_0670", mode="overwrite"
)
