...
tra_dbf_0540_main = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0540_main"
)
tra_dbf_0540_karnrresult = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0540_karnrresult"
)
tra_dbf_0540_korrelation = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0540_korrelation"
)
tra_dbf_0540_additionalchecks = spark.table(
    f"`{catalog}`.`{transformation_schema}`.tra_dbf_0540_additionalchecks"
)
# tra_dbf_054_mrs = spark.table(f"`{catalog}`.`{transformation_schema}`.tra_dbf_054_mrs")


# COMMAND ----------

# Test data source
tra_dbf_0540_main.display()
...
t1 = (
    tra_dbf_0540_main.filter(col("ingest_date") >= date_sub(current_date(), 90))
...
mrt_dbf_0540 = (
    t1.join(
        tra_dbf_0540_karnrresult.alias("t2"),
        (
    ...
    .join(
        tra_dbf_0540_korrelation.alias("t3"),
    ...
    .join(
        tra_dbf_0540_additionalchecks.alias("t4"),
...
mrt_dbf_0540.display()

# COMMAND ----------

# Select final fields and cast types
mrt_dbf_0540_final = mrt_dbf_0540.select(
    "anlage_subtyp",
    "timestamp",
...
# Final union with tra_dbf_050_mrs --> mrs not yet created
# result_df = mrt_dbf_054_final.unionByName(tra_dbf_054_mrs)
result_df = mrt_dbf_0540_final


# COMMAND ----------

check_df = result_df.filter(col("source") == "mrs")
check_df.display()
...
result_df.write.saveAsTable(
    f"`{catalog}`.`{marts_schema}`.mrt_dbf_0540", mode="overwrite"
)
