...
tb_einrichtpunkte_baureihe_dbf_0540 = tb_einrichtpunkte_baureihe_dbf_0540.withColumn(
    "punkt",
...
join_condition_sma = (
    (
        processed_iqvis_filtered_sma["messwerttyp"]
        == tb_einrichtpunkte_baureihe_dbf_0540["messwerttyp"]
    ) &
    (
        processed_iqvis_filtered_sma["fzg_typ"]
        == tb_einrichtpunkte_baureihe_dbf_0540["fzg_typ"]
    )
    & (
        processed_iqvis_filtered_sma["origin"]
        == tb_einrichtpunkte_baureihe_dbf_0540["origin"]
    )
)
...
join_result_sma = (
    ...
    .join(tb_einrichtpunkte_baureihe_dbf_0540, join_condition_sma, "left")
    .distinct()
)
...
join_condition_bfk_sme = (
    (
        processed_iqvis_filtered_bfk_sme["messwerttyp"]
        == tb_einrichtpunkte_baureihe_dbf_0540["messwerttyp"]
    ) &
    (
...
join_result_bfk_sme = (
    ...
    .join(tb_einrichtpunkte_baureihe_dbf_0540, join_condition_bfk_sme, "left")
    .distinct()
)
...
tb_final.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_0540_karnrresult", mode="overwrite"
)
