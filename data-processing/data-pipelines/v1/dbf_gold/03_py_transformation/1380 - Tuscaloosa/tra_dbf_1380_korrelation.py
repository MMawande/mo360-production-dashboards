...
tb_iqvis_tra_1 = (
    ...
    .withColumn("messwerttyp_calc", messwerttyp_case(F.col("messwerttyp")))
)
...
tb_iqvis_tra_2 = (
    ...
    .withColumn(
        ...
        .when(F.col("uniqueid").like("%SME%"), "Assembly")
        .otherwise("Rest"),
    )
    .withColumn("messwerttyp_calc", messwerttyp_case(F.col("messwerttyp")))
)
...
final_df.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_1380_korrelation", mode="overwrite"
)
