...
# Load tables (from hive schema) --> should be replace with table from unity schema
...
tb_einrichpunkte = tb_einrichtpunkte_baureihe_dbf_1380.select(
    col("messwerttyp"),
    col("fzg_typ"),
    col("origin"),
...
df_joined = (
    ...
    .join(
        ...
        & (col("iq.messwerttyp_calc") == col("ep.messwerttyp")),
        "left",
    )
    .join(
        tb_prodnumber.alias("pn"),
        (col("iq.fzg_typ_calc") == col("pn.fzg_typ")) & (col("iq.karnr") == col("pn.karnr")),
        "left",
    )
)
...
df_joined = df_joined.withColumn(
    "header_measurement_meastime", from_utc_timestamp(col("iq.timestamp"), "CET")
)
...
df_joined = df_joined.withColumn(
    "ingest_date",
    (
        from_utc_timestamp(col("iq.timestamp"), "CET")
        + expr("INTERVAL 1 HOUR 50 MINUTE")
    ).cast("date"),
)
...
df_final.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_1380_main", mode="overwrite"
)
