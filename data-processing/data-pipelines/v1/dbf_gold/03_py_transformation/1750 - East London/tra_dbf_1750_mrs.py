...
joined_filtered_df = (
    ...
    .filter(
        (col("t1.plant") == "1750")
        & (col("t2.plant") == "1750")
        & (col("t1.checkpoint_description").isin("ASSEMBLY START/EXIT BUFFER"))
...
result_df = joined_filtered_df.select(
    ...
    when(
        (col("t2.vehicle_type").isin("W206"))
        & (col("t1.checkpoint_description") == "ASSEMBLY START/EXIT BUFFER"),
        "Assembly",
    )
    .when(
        (col("t2.vehicle_type").isin("V295"))
        & (col("t1.checkpoint_description") != "ASSEMBLY START/EXIT BUFFER"),
        "Bodyshop",
    )
    .alias("origin"),
...
result_df.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_1750_mrs", mode="overwrite"
)
