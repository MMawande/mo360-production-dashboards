...
tb_einrichtpunkte_baureihe_dbf_0500 = tb_einrichtpunkte_baureihe_dbf_0500.withColumn(
    "punkt",
...
join_condition_sma = (
    (
        ...
        == tb_einrichtpunkte_baureihe_dbf_0500["messwerttyp"]
    ) &
    (
        processed_iqvis_filtered_sma["fzg_typ"]
        == tb_einrichtpunkte_baureihe_dbf_0500["fzg_typ"]
    )
    & (
        processed_iqvis_filtered_sma["origin"]
        == tb_einrichtpunkte_baureihe_dbf_0500["origin"]
    )
)
...
join_result_sma = (
    ...
    .join(tb_einrichtpunkte_baureihe_dbf_0500, join_condition_sma, "left")
    .distinct()
)
...
join_result_bfk_sme = (
    ...
    .join(tb_einrichtpunkte_baureihe_dbf_0500, join_condition_bfk_sme, "left")
    .distinct()
)
...
# Perform union
iqvis_final = iqvis_final = join_result_sma.unionByName(join_result_bfk_sme)
...
tb_final.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_0500_karnrresult", mode="overwrite"
)
