...
processed_iqvis = processed_iqvis.withColumn(
    "fzg_typ_calc",
    when(col("anlage_subtyp").like("%BFK%"), split(col("anlage_subtyp"), "_")[1])
    .when(col("anlage_subtyp").like("%AMG%"), split(col("anlage_subtyp"), "_")[0])
    .when(col("anlage_subtyp").like("%SMA%"), split(col("anlage_subtyp"), "_")[0])
    .when(col("anlage_subtyp").like("%SME%"), split(col("anlage_subtyp"), "_")[1]),
)

# COMMAND ----------


# MAGIC ##Logic for `origin`
...
# MAGIC #tb_einrichtpunkte

# COMMAND ----------

# DBTITLE 1,Transform logic
tb_einrichpunkte = tb_einrichtpunkte_baureihe_dbf_0500.select(
    col("messwerttyp"),
    col("fzg_typ"),
    col("origin"),
...
# tb_minamount = tb_einrichtpunkte_baureihe_dbf_138

# Compute tb_minamount
tb_minamount = (
    tb_einrichtpunkte_baureihe_dbf_0500.filter(
...
catalog = f"westeurope_mo360dp_usecases_{env_stage}"
trasnformation_schema = "proddshbrd_01_emea_dbf_transformation"

# save as table in unity catalog
final_df.write.saveAsTable(
    f"`{catalog}`.`{trasnformation_schema}`.tra_dbf_0500_additionalchecks",
    mode="overwrite",
)
