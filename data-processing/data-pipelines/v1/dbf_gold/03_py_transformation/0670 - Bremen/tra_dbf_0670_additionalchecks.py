...
tb_einrichtpunkte_baureihe_dbf_0670 = spark.table(
    f"`{catalog}`.`{mapping_schema}`.tb_einrichtpunkte_baureihe_dbf_0670"
)
...
tb_einrichpunkte = tb_einrichtpunkte_baureihe_dbf_0670.select(
    col("messwerttyp"),
    col("fzg_typ"),
    col("origin"),
...
tb_minamount = (
    tb_einrichtpunkte_baureihe_dbf_0670.filter(
...
final_df.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_0670_additionalchecks", mode="overwrite"
)
