    ...
    .filter(
        ...
        & (
            col("t1.checkpoint_description").isin(
                "H9 EINLAUF BD54",
                "H3/10PC SC-NA ST9O B23",
                "H7 M1 MA42 710 Auslauf",
                "H70 Z3 FT No Touch QTor",
                "MA51-160 SRS060 FAT GEO",
                "H36 IS HUD NA UND LINIE",
            )
        )
...
result_df = joined_filtered_df.select(
    ...
    when(
        (col("t2.vehicle_type").isin("X254", "V295", "S206", "C254"))
        & (col("t1.checkpoint_description") == "H9 EINLAUF BD54"),
        "SMA-MO",
    )
    .when(
        (col("t2.vehicle_type").isin("X254", "C254", "A205", "C205"))
        & (col("t1.checkpoint_description") != "H9 EINLAUF BD54"),
        "MA21/22-RB",
    )
    .when(
        (col("t2.vehicle_type").isin("A238", "C238"))
        & (col("t1.checkpoint_description") != "H9 EINLAUF BD54"),
        "MA11/12-RB",
    )
    .when(
        (col("t2.vehicle_type").isin("W206", "S206", "A236", "C236"))
        & (col("t1.checkpoint_description") != "H9 EINLAUF BD54"),
        "MA41/42-RB",
    )
    .when(
        (col("t2.vehicle_type") == "V295")
        & (col("t1.checkpoint_description") != "H9 EINLAUF BD54"),
        "MA51/52-RB",
    )
    .alias("origin"),
...
result_df.write.saveAsTable(
    f"`{catalog}`.`{processed_schema}`.tra_dbf_0670_mrs", mode="overwrite"
)
